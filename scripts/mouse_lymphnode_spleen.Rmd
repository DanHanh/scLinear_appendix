...
for(n in names(mls.data)){
  ...
  mls <- Seurat::CreateSeuratObject(counts = tmp[["Gene Expression"]], min.cells = 1, min.features = 1)
...
for(n in names(mls_list)){
  ...
  iso_features <- c("Isotype-G0114F7", "Isotype-HTK888", "Isotype-MOPC.173", "Isotype-MOPC.21", "Isotype-MPC.11", "Isotype-RTK2071", "Isotype-RTK2758", "Isotype-RTK4174", "Isotype-RTK4530")
...
LymphNode1_vs_LymphNode2 <- get_bit_table(train_set, test_set) %>% mutate(train_test = "LymphNode1_vs_LymphNode2")
train_set <- mls_list$LymphNode2
test_set <- mls_list$LymphNode1
LymphNode2_vs_LymphNode1 <- get_bit_table(train_set, test_set) %>% mutate(train_test = "LymphNode2_vs_LymphNode1")
# train vs test Spleen
train_set <- mls_list$Spleen1
test_set <- mls_list$Spleen2
Spleen1_vs_Spleen2 <- get_bit_table(train_set, test_set) %>% mutate(train_test = "Spleen1_vs_Spleen2")
train_set <- mls_list$Spleen2
test_set <- mls_list$Spleen1
Spleen2_vs_Spleen1 <- get_bit_table(train_set, test_set) %>% mutate(train_test = "Spleen2_vs_Spleen1")
...
pipe <- fit_predictor(pipe = pipe,
 gexp_train = train_set@assays[["RNA"]],
              adt_train = train_set@assays[["ADT"]],
              normalize_gex = TRUE,
              normalize_adt = TRUE)

eval_res <- evaluate_predictor(pipe, test_set@assays$RNA, test_set@assays$ADT, normalize_gex = TRUE, normalize_adt = TRUE)

predicted_assay <- adt_predict(pipe = pipe, gexp = test_set@assays[["RNA"]], normalize = TRUE)
test_set[["predicted_adt"]] <- predicted_assay

adt_real <- as.matrix(test_set@assays[["ADT"]]@data)
adt_real <- adt_real %>% as.data.frame() %>% rownames_to_column("gene") %>% pivot_longer(!gene,names_to = "cell", values_to = "real")
adt_predicted_sclinear <- as.matrix(test_set@assays[["predicted_adt"]]@data)
adt_predicted_sclinear <- adt_predicted_sclinear %>% as.data.frame() %>% rownames_to_column("gene") %>% pivot_longer(!gene,names_to = "cell", values_to = "prediced_adt")


meta <- test_set@meta.data %>% rownames_to_column("cell")
...
df <- DF %>% na.omit()  %>% group_by(gene)  %>%
      summarize('scLinear vs ADT' = cor(real, predicted_adt, method = "pearson"))
...
df$name <- df$gene
df$name[!(df$gene %in% c("CD11b", "CD19","CD3", "CD27"))] <- ""
...
cors <- PBMC %>% group_by(gene) %>% dplyr::summarise(correlation = cor(real, predicted_adt))
ggplot(cors, aes(x=reorder(gene, correlation), y= correlation,fill=correlation)) + geom_bar(stat="identity", col="black") + coord_flip() +
    theme_classic2() + scale_fill_gradientn(colours = inferno(11)) + ylab("Pearson\n(Real vs ScLinear)") + xlab("Protein") + theme(legend.position = "none") +
