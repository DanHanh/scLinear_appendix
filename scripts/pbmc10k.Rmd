...
d <- DF %>% na.omit() %>% group_by(gene)  %>% summarize(pearson_pADT_vs_rADT = cor(real, predicted_sclinear, method = "pearson"), pearson_normalizedRNA_vs_rADT = cor(real, rna_normalized, method = "pearson"), pearson_rawRNA_vs_rADT = cor(real, rna_raw, method = "pearson")) #%>% ungroup() %>% summarize(pearson = mean(pearson), spearman = mean(spearman))
...
p <- ggplot(d, aes(x = gene, y = pearson, fill = comparison)) + geom_bar(stat="identity", position=position_dodge()) + theme_bw() + scale_fill_manual(values = kelly()[4:length(kelly())])
p

ggsave(filename = "./../local/RNA_ADT_correlation.pdf", plot = p, width = 15, height = 10)
```
